...
export default {
  ...
  methods: {
    async enable2FA() {
      ...
      await this.$axios.$post('user/two-factor-authentication?portal=company')

      this.qrCode = (await this.$axios.$get('user/two-factor-qr-code?portal=company')).svg
    },
    async disable2FA() {
      await this.$axios.$get('sanctum/csrf-cookie')
      await this.$axios.$delete('user/two-factor-authentication?portal=company')
      await this.$auth.fetchUser();
    },
    async confirm2FA() {
      try {
        await this.$axios.$get('sanctum/csrf-cookie')
        await this.$axios.$post('user/confirmed-two-factor-authentication?portal=company', this.confirm_2fa_form.fields)
        await this.$auth.fetchUser()
        this.qrCode = ''
    ...
    {
      await this.$axios.$get('sanctum/csrf-cookie')
      this.recover_codes = (await this.$axios.$get('user/two-factor-recovery-codes?portal=company'))
    }
  }
}
</script>
