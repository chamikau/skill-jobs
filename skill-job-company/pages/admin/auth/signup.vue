<template>
  <div class="page page-center">
    <div class="container-tight py-4">
      <div class="text-center mb-4"><a class="navbar-brand navbar-brand-autodark" href="."><img src="@/assets/moxy_logo_final.png" alt=""
                                                                                                height="36"></a>
      </div>
      <div class="mt-1">
        <form class="card card-md" @submit.prevent="signup">
          <div class="card-body">
            ...
            <div class="mb-3">
              <label class="form-label"
                     :class="[signup_form.errors.email ? 'text-danger' : '']">Email Address</label>
            ...
            <div class="col-md-12" v-if="is_company">
              <div class="mt-0">
                <label
                  class="form-label"
                  :class="[signup_form.errors['company.name']? 'text-danger' : '']">Company Name</label>
                <input
...
export default {
  ...
  methods: {
    ...
    clearValidationCompanyNameError() {
      this.signup_form.errors['company.name'] = false;
    },
    clearValidationAboutCompanyError() {
      this.signup_form.errors['company.about_the_company'] = false;
    },
    async signup() {
      try {
        this.signup_form.fields.has_company = this.is_company
        await this.$axios.$get('sanctum/csrf-cookie')
        await this.$axios.$post('/api/company/register?portal=company', this.signup_form.fields);
    ...
    async getCities() {
      try {
        let attributes = await this.$axios.$get('/api/company/cities');
    ...
    selectCity() {
      ...
      // this.signup_form.errors['company.city_id'] =''
    },
    async getIndustryTypes() {
      try {
        let attributes = await this.$axios.$get('/api/company/industry-types');
    ...
    selectIndustryTypes() {
      ...
      // this.signup_form.errors['company.industry_type_id'] =''
    },
  },
  mounted() {
    this.getCities();
