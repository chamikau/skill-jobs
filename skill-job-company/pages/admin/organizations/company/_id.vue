...
export default {
  ...
  methods: {
    clearValidationNameError() {
      this.update_company_form.errors.name = false;
    },
    clearValidationAboutCompanyError() {
      this.update_company_form.errors.about_the_company = false;
    },
    clearValidationIndustryTypesError() {
      this.update_company_form.errors.industry_type_id = false;
      this.selectComponentKey++
    },
    clearValidationCityError() {
      this.update_company_form.errors.city_id = false;
      this.selectComponentKey++
    },
    async updateCompanyData() {
      try {
        let company = await this.$axios.$put(`/api/company/organizations/${this.$route.params.id}`, this.update_company_form.fields)
    ...
    async getActiveCompany(company) {
        try {
          if (company.state === 'inactive') {
            await this.$axios.$post(`/api/company/organizations/${this.$route.params.id}/active`)
            this.$toast.success('Active')
          } else if (company.state === 'active') {
            await this.$axios.$post(`/api/company/organizations/${this.$route.params.id}/inactive`)
    ...
    async getOrganization() {
      try {
        this.company = (await this.$axios.$get(`/api/company/organizations/${this.$route.params.id}`, this.update_company_form.fields)).data
    ...
    async reset() {
      try {
        this.company = (await this.$axios.$get(`/api/company/organizations/${this.$route.params.id}`)).data
        this.update_company_form.fields = this.company
        this.cityAttributes.selected = this.cityAttributes.data.find((x)=>x.value === this.company.city_id);
        this.industryTypeAttributes.selected = this.industryTypeAttributes.data.find((x)=>x.value === this.company.industry_type_id);
      }catch (e){
